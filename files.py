...
__date__      = "$LastChangedDate: 2012-01-29 22:52:35 +0100 (Sun, 29 Jan 2012) $"
...
class MyPrincetonSPEFile():
    ...
    def __init__(self, fname = None, fid = None):
        ...
        if self._fid:
#            self.readData()
            self.read_everything_except_the_actual_data()
    ...
    def read_everything_except_the_actual_data(self):
        self._readHeader()
        self._readSize()
        self._readComments()
        self._readAllROI()
        self._readDate()
        # anticipate the user to call return_Array() next,
    ...
    def return_Array(self):
        self._fid.seek(self.DATASTART)
        return numpy.fromfile(self._fid, dtype = self._dataType, count = -1).reshape(self._size)
